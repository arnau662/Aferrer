...
function iniciar_sesion($mail, $password, $nom) {
    if (comprovar_email($mail) && comprovar_contra($password)) {
        $password = md5($password);
        $conn = new mysqli("localhost", "aferrer", "aferrer", "aferrer_dbprova");
                
        $sql = "SELECT * FROM usuaris_examen WHERE username='$mail' and password='$password'";
...
function comprovar_email_db($user) {
    if (comprovar_email($user)) {
        ...
        $sql = "SELECT * FROM usuaris_examen WHERE username='$user'";
        $result = $conn->query($sql);
...
function cambiar_password($password, $user, $codi) {
    ...
    $sql = "UPDATE usuaris_examen SET password='$password' WHERE username='$user'";
    $result = $conn->query($sql);
